"""
Adapted from https://github.com/LetheSec/Fer2013-Facial-Emotion-Recognition-Pytorch
"""
...
parser.add_argument(
    "--data_path",
    default="/home/nero-ia/Documents/Boscatti/EmotionDetection/train/datasets/fer2013/fer2013.csv",
    type=str,
)
...
def main():
    ...
    checkpoint_path = os.path.join(args.results, args.name, args_path, "checkpoints")

    if not os.path.exists(checkpoint_path):
        os.makedirs(checkpoint_path)

    writer = SummaryWriter(
        os.path.join(args.results, args.name, args_path, "tensorboard_logs")
    )

    logger = Logger(os.path.join(args.results, args.name, args_path, "output.log"))
    ...
    train_loader, val_loader, test_loader = get_dataloaders(
        path=args.data_path, bs=args.batch_size, augment=True
    )
    ...
    if args.resume > 0:
        ...
        state_dict = torch.load(
            os.path.join(checkpoint_path, "checkpoint_" + str(args.resume) + ".tar")
        )
        model.load_state_dict(state_dict["model_state_dict"])
        optimizer.load_state_dict(state_dict["opt_state_dict"])

    logger.info("Start traning.")
    logger.info("Epoch \t Time \t Train Loss \t Train ACC \t Val Loss \t Val ACC")
