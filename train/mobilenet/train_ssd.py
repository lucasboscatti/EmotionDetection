...
parser.add_argument(
    "--datasets", "--data", nargs="+", default=["data"], help="Dataset directory path"
)
parser.add_argument(
    "--balance-data",
    action="store_true",
...
parser.add_argument(
    "--freeze-base-net", action="store_true", help="Freeze base net layers."
)
parser.add_argument(
    "--freeze-net",
    action="store_true",
...
# Params for SGD
parser.add_argument(
    "--lr", "--learning-rate", default=0.01, type=float, help="initial learning rate"
)
parser.add_argument(
    "--momentum", default=0.9, type=float, help="Momentum value for optim"
)
parser.add_argument(
    "--weight-decay", default=5e-4, type=float, help="Weight decay for SGD"
)
...
parser.add_argument(
    "--num-epochs", "--epochs", default=30, type=int, help="the number epochs"
)
parser.add_argument(
    "--num-workers",
    "--workers",
...
parser.add_argument(
    "--debug-steps", default=10, type=int, help="Set the debug log output frequency."
)
parser.add_argument(
    "--use-cuda", default=True, action="store_true", help="Use CUDA to train model"
)
parser.add_argument(
    "--checkpoint-folder",
    "--model-dir",
...
def train(loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    tensorboard.add_scalar(
        "Classification Loss/train", train_classification_loss, epoch
    )
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = get_dataloaders(
        path="/Users/lucas.boscatti/Documents/nero/emotion_detection/train/datasets/fer2013/fer2013.csv",
        bs=64,
        augment=True,
    )
    ...
    if args.resume:
        ...
        if (
            not os.path.exists(args.pretrained_ssd)
            and args.pretrained_ssd == DEFAULT_PRETRAINED_MODEL
        ):
            os.system(
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            if args.validation_mean_ap:
                ...
                for i in range(len(class_ap)):
                    tensorboard.add_scalar(
                        f"Class Average Precision/{eval_dataset.class_names[i+1]}",
                        class_ap[i],
                        epoch,
                    )
